	...
	private void writePackets(Thread thisThread) {
		...
		try {
			...
			while (!done && (writerThread == thisThread)) {
				...
				if (packet != null) {
					synchronized (writer) {
						Log.i("XMPP", "--send:" + packet.toXML());
						writer.write(packet.toXML());
						connection.firePacketSendListenersSuccess(packet);
						writer.flush();
			...
			// error occur when send packet...
			if (connection != null) {
				if (packet != null)
					connection.firePacketSendListenersFailed(packet);
				while (!queue.isEmpty()) {
					packet = queue.remove();
					connection.firePacketSendListenersFailed(packet);
				}
				...
				synchronized (writer) {
					...
					if (System.currentTimeMillis() - lastActive >= delay) {
						try {
							...
							// Do nothing
							// notify the XMPPconnection the socket has disconnect.not do nothing
							// add by Elvis
							shutdown();
