...
class PacketReader {
    ...
    private void parsePackets(Thread thread) {
        try {
            ...
            do {
            	// Logger.i("element  type="+eventType);
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    // the connectionID lock so that the packet reader startup can finish.
                    	...
                    	//stream error:stream:error (conflict)
                    	
                    	if(streamError!=null&&streamError.toString().contains("conflict")){
                    		connection.notifyAccountConflect();
                    	}
            ...
            //Logger.i("end because: done?"+done+", eventType="+eventType+" thread ok?"+(thread == readerThread));
        }
        catch (Exception e) {
        	 //Logger.i("exception occur");
        	e.printStackTrace();
        	 if (!done||e instanceof IOException) {
    ...
    private void processPacket(Packet packet) {
        ...
        Log.i("XMPP", "--receive:"+packet.toXML());
        //Logger.i("--receive:"+packet.toXML());
