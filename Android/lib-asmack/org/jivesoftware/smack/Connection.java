...
public abstract class Connection {
	 ...
	 */
	protected final Map<PacketSendListener, SendListenerWrapper> sendListeners = new ConcurrentHashMap<PacketSendListener, SendListenerWrapper>();
	...
	//protected final Collection<PacketSendListener> packetSendListeners = new CopyOnWriteArrayList<PacketSendListener>();
	 ...
	 * by the server, then sets presence to available. If more than five seconds
	 * (default timeout) elapses in each step of the authentication process
	 * without a response from the server, or if an error occurs, a
	 * XMPPException will be thrown.
	 * <p>
	 * 
	 ...
	 *            the packet filter to use.
	 */
	public void addPacketSendingListener(PacketSendListener packetListener, PacketFilter packetFilter) {
		if (packetListener == null) {
			throw new NullPointerException("Packet listener is null.");
		}
		SendListenerWrapper wrapper = new SendListenerWrapper(packetListener, packetFilter);
	 ...
	 */
	protected Map<PacketSendListener, SendListenerWrapper> getPacketSendingListeners() {
		return sendListeners;
	}

	protected void firePacketSendListenersSuccess(Packet packet) {
		// Notify the listeners of the new sent packet
		for (SendListenerWrapper listenerWrapper : sendListeners.values()) {
			listenerWrapper.notifySuccess(packet);
		}
	}

	protected void firePacketSendListenersFailed(Packet packet) {
		// Notify the listeners of the new sent packet
		for (SendListenerWrapper listenerWrapper : sendListeners.values()) {
			listenerWrapper.notifyError(packet);
		}
	}
	/**
	 ...
	 */
	protected static class SendListenerWrapper {
		 ...
		 *            packets.
		 */
		public SendListenerWrapper(PacketSendListener packetListener, PacketFilter packetFilter) {
			this.packetListener = packetListener;
			this.packetFilter = packetFilter;
		}

		public void notifyError(Packet packet) {
			if (packetFilter == null || packetFilter.accept(packet))
				packetListener.onPacketSendFailed(packet);
		}

		public void notifySuccess(Packet packet) {
			if (packetFilter == null || packetFilter.accept(packet))
				packetListener.onPacketSendSuccess(packet);
		}
	}
     ...
     */
    protected void notifyAccountConflect() {
        for (ConnectionListener listener : connectionListeners) {
            try {
                listener.onAccountConflicted();
            }
            catch (Exception e) {
