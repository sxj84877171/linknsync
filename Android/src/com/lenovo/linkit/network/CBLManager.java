 ...
 */
public class CBLManager implements INetworkManager, Replication.ChangeListener {

	private static final String TAG = "CBLManager";
	...
	public CBLManager(Context context, URI svrUri, Handler handler) {
	...
	protected void startCBLite() throws Exception {
		...
		// create cmd document for web using it to send the the command
	//	createCmdDoc();
	...
	private boolean processCmdDoc(Document doc){
		...
		if (null != sMsg && !sMsg.isEmpty()) {
			String sChannels = (String) currentRevision.getProperty(CBL_KEY_CHANNELS);			
	...
	private boolean processCommand(Document doc){
		if (null == doc){
			return false;
		}
		if (doc.getId().equalsIgnoreCase(mCmdDocId)){			
			try {
				List<SavedRevision> lst = doc.getRevisionHistory();
				for(SavedRevision currentRevision: lst){
					if (null == currentRevision){
						continue;
					}
					long seq = currentRevision.getSequence();
					mCmdSeq = UserSetting.getLastSeq(mContext, 0);
					if (seq <= mCmdSeq){
						continue;
					}
					String sMsg = (String) currentRevision.getProperty(CBL_KEY_PKG);
					if (null != sMsg && !sMsg.isEmpty()) {
						Packet packet = Packet.parse(sMsg);

						mCmdSeq = seq;
						UserSetting.setLastSeq(mContext, mCmdSeq);
						// 过滤掉冗余的历史记录,处理web端发来的命令
						if (null != packet) {
							try {
								Engine.getEngine().processCommand(sMsg);
							} catch (Exception e) {
	...
	private boolean processRefresh(Document doc){
		if (null == doc){
			return false;
		}
		SavedRevision currentRevision = doc.getCurrentRevision();
		...
		String sStatus = (String) currentRevision
				.getProperty(CBL_KEY_STATUS);

    	//Map<String, Object> newProperties2 = new HashMap<String, Object>(currentRevision.getProperties());
		// 处理二维码页面刷新情况
		if (null != sStatus && !sStatus.isEmpty()){
			if (sStatus.equalsIgnoreCase("0")){
            	Map<String, Object> newProperties = new HashMap<String, Object>(currentRevision.getProperties());
            	mChannels.remove(mDataChannelId);
            	mDataChannelId = UUID.randomUUID().toString();
            	
            	newProperties.put(CBL_KEY_DATA_CHANNEL, mDataChannelId);
            		while(true){
                    	try {
							doc.putProperties(newProperties);
                        	mChannels.add(mDataChannelId);
                        	startSync();	
                        	sendSMS();
                        	break;
						} catch (CouchbaseLiteException e) {
	...
	private Document createHandshakeDoc() throws Exception {
		Document document = database.getDocument(mHandShakeDocId);
		if(null != document){
			document.update(new Document.DocumentUpdater() {
			    @Override
			    public boolean update(UnsavedRevision newRevision) {
			        Map<String, Object> properties = newRevision.getProperties();
			        properties.put(CBL_KEY_CHANNELS, mHandShakeDocId);
			        properties.put(CBL_KEY_CREATE, getCurTime());
					properties.put(CBL_KEY_STATUS, "1");
					properties.put(CBL_KEY_DATA_CHANNEL, mDataChannelId);
			        newRevision.setProperties(properties);
			        FLog.i(TAG, "createHandshakeDoc document.update finished");
	...
	private Document createCmdDoc() throws Exception {
		
		Document doc = database.getExistingDocument(mCmdDocId);
		Document document = database.getDocument(mCmdDocId);
		if(null != document){
			document.update(new Document.DocumentUpdater() {
			    @Override
			    public boolean update(UnsavedRevision newRevision) {
			        Map<String, Object> properties = newRevision.getProperties();
			        properties.put(CBL_KEY_CHANNELS, mCmdDocId);
			        properties.put(CBL_KEY_CREATE, getCurTime());
					properties.put(CBL_KEY_PKG, "");
			        newRevision.setProperties(properties);
			        FLog.i(TAG, "createCmdDoc document.update finished");
			        return true;
			    }
			});
		}
		
		//saveDocID(document.getId());
		return document;
	}
	...
	private Document sendPackage(String sPkg) throws Exception {

		Document document = database.createDocument();
		Map<String, Object> properties = new HashMap<String, Object>();

		properties.put(CBL_KEY_CHANNELS, mDataChannelId);
		properties.put(CBL_KEY_PKG, sPkg);
       // properties.put(CBL_KEY_CREATE, getCurTime());
	...
	public void changed(Replication.ChangeEvent event) {
		...
		FLog.d(TAG, "Replication : " + replication + " changed.");
...
//	    mCmdDocId = getQRCode() + "_cmd";
		mChannels.add(mCmdChannelId);
//		mChannels.add(mHandShakeDocId);
//		mChannels.add(mDataChannelId);
	}
    	 ...
    	 new Thread(new Runnable() {
			 public void run() {
					try {
						...
						HandShakePkg pkg = gson.fromJson(content, HandShakePkg.class);
    ...
    private String sendPackRest(final String msg){
   	 new Thread(new Runnable() {
			 public void run() {
					try {
						Gson gson = new Gson();
						String url = mSyncUri.toString() + "/" + UUID.randomUUID().toString();
